### Extension note: ###
#
# Besides the three other models provided in the original FixMatch code, we extended this file by a MC-Dropout variant
# called ResNetMCDropout. Accordingly, MultiModel has been extended by the respective model class.
#
...
class ResNetMCDropout(ClassifySemi):
    def classifier(self, x, scales, filters, repeat, training, getter=None, dropout=0, prob=1.0, **kwargs):
        del kwargs
        leaky_relu = functools.partial(tf.nn.leaky_relu, alpha=0.1)
        bn_args = dict(training=training, momentum=0.999)
        ...
        def residual(x0, filters, stride=1, activate_before_residual=False):
            ...
            x = tf.layers.conv2d(x, filters, 3, strides=stride, **conv_args(3, filters))
            x = tf.nn.dropout(x, prob, name="dropout_layer")
            x = leaky_relu(tf.layers.batch_normalization(x, **bn_args))
            x = tf.layers.conv2d(x, filters, 3, **conv_args(3, filters))
...
class MultiModel(CNN13, ResNet, ShakeNet):
    MODELS = ('cnn13', 'resnet', 'shake', 'mcdropresnet')
    MODEL_CNN13, MODEL_RESNET, MODEL_SHAKE, MODEL_MCDROPRESNET = MODELS
    ...
    def classifier(self, x, arch, **kwargs):
        ...
        elif arch == self.MODEL_MCDROPRESNET:
            return ResNetMCDropout.classifier(self, x, **kwargs)
